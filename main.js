...
mobile: MethodMobile, 
browser: opcion == '1' ? ['MonkeyBot', 'Edge', '2.0.0'] : methodCodeQR ? ['MonkeyBot', 'Edge', '2.0.0'] : ['Chrome (Linux)', '', ''],
auth: {
creds: state.creds,
...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "MonkeyBot"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
...
const directories = ['./TonyBot/', './GataJadiBot/']
